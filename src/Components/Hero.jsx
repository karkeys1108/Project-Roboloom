...
const Hero = () => {
  ...
  // Predefined theme colors for gradient balls
  const themeColors = [
    "#7586E8, #ADB8DA", 
    "#EDE8F5, #8697C4",
    "#B3C5E4, #A0B0C7", 
  ...
  // Fixed positions for the gradient balls
  ...
  // Fixed sizes for gradient balls (small, medium, large)
  const fixedSizes = [
    "20px",  // Small
    "30px",  // Medium
    "40px",  // Large
  ...
  // Create an array of gradient balls with fixed positions, sizes, and colors
  const gradientBalls = fixedPositions.map((position, index) => ({
    size: fixedSizes[index % fixedSizes.length], // Use fixed sizes based on index
    position,
  ...
  // Variants for sliding in from bottom
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p className="mt-4 md:mt-6 font-robotoFlex text-gray-800 text-sm md:text-base">
          At <span className="font-quicksand font-semibold text-[#8697C4]">RoboLoom</span>, we inspire creativity in students through robotics. Our hands-on courses spark curiosity, foster innovation, and equip young learners with essential tech skills.
        </p>
      ...
      >
        ...
        <motion.img src={Brain} alt="Brain"
          ...
          className="absolute top-[-5%] left-[15%] w-[70px] md:w-[85px] opacity-90 z-20 rotate-[-10deg]"
        />
